...
async def run_shopify_test():
    ...
    # 1. Setup Settings (loads .env and AKV automatically)
    print("⚙️ Loading Settings...")
    # Explicitly point to .env in current dir
    ...
    if not settings.OPENAI_API_KEY:
        print("❌ Error: OPENAI_API_KEY missing from .env and AKV.")
        return
    ...
    try:
        ...
        if not remote_tools:
            print("⚠️ Warning: No tools discovered. Is the Node.js hub running?")
            print("   Run 'npm run start' in 'apps/shopify-mcp' first.")
            return
    ...
    # 4. Run Test Query
    query = "Search for some cool products and tell me their prices."
    print(f"\n💬 Query: {query}")
    print("⏳ Processing...")
    ...
    # 5. Test Customer Protected Tool (Should trigger AuthRequired)
    print("\n👤 Testing Customer Tool (Expected to trigger Auth link)...")
    ca_query = "What is my profile email?"
    print(f"💬 Query: {ca_query}")
    
    ca_result = await orchestrator.run(ca_query)
    print("\n📝 CA ANSWER:")
    print(ca_result.answer)
